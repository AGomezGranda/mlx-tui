...
from mlx_tui.sessions.codec import MAX_AGGREGATE_BYTES, decode_session, encode_session
...
from mlx_tui.sessions.queries import request_messages, successful_turns
...
needs_lock = pytest.mark.skipif(fcntl is None, reason="session locking is Unix-only")
needs_fork = pytest.mark.skipif(
    fcntl is None or not hasattr(os, "fork"),
...
def _turn(**overrides: object) -> SessionTurn:
    base: dict[str, object] = {
    ...
    base.update(overrides)
    return SessionTurn(**base)  # type: ignore[arg-type]
...
def _session(**overrides: object) -> ChatSession:
    ...
    base.update(overrides)
    return ChatSession(**base)  # type: ignore[arg-type]


def _raw_session_doc(session_id: str) -> dict[str, object]:
    return json.loads(encode_session(_session(session_id=session_id)))
...
def test_success_only_derived_history() -> None:
    ...
    path = save_session(session)
    with lock_session(session.session_id):
        recovered = load_session(path)
    assert request_messages(recovered) == [
        {"role": "user", "content": "q1"},
        {"role": "assistant", "content": "a1"},
    ]
    assert [turn.outcome for turn in successful_turns(recovered)] == ["success"]
...
def test_v1_sessions_upgrade_to_v2_with_empty_attachments() -> None:
    ...
    upgraded = decode_session(json.dumps(document), expected_id=session_id)

    assert upgraded.schema_version == 2
    assert upgraded.attachments == ()
    encoded = json.loads(encode_session(upgraded))
...
def test_attachment_aggregate_bound_counts_all_draft_files(tmp_path: Path) -> None:
    ...
    with pytest.raises(SessionValidationError, match="1 MiB"):
        encode_session(_session(attachments=snapshots))
...
def test_strict_turn_rejection() -> None:
    ...
    def _turn_from_doc() -> dict[str, object]:
        return json.loads(encode_session(_session(attempts=(_turn(),))))["attempts"][0]
...
def test_corrupt_and_newer_files_stay_intact() -> None:
    ...
    assert load_session(good_path) == good
...
def test_filename_uuid_mismatch_rejected() -> None:
    ...
    other = session_dir() / f"{uuid.uuid4()}.json"
    other.write_text(encode_session(session), encoding="utf-8")
...
def test_duplicate_attempt_ids_and_bad_references_rejected() -> None:
    ...
    with pytest.raises(SessionValidationError, match="unique"):
        encode_session(_session(attempts=(first, dupe)))
    ...
    with pytest.raises(SessionValidationError, match="successful"):
        encode_session(_session(attempts=(failed, later_ref)))
    ...
    with pytest.raises(SessionValidationError, match="prior"):
        encode_session(_session(attempts=(forward, second)))

    missing = dataclasses.replace(
        first, included_turn_ids=(str(uuid.uuid4()),), created_at=T0
    )
    with pytest.raises(SessionValidationError, match="prior"):
        encode_session(_session(attempts=(missing,)))

    repeated = dataclasses.replace(
        second, included_turn_ids=(first.turn_id, first.turn_id)
    )
    with pytest.raises(SessionValidationError, match="unique"):
        encode_session(_session(attempts=(first, repeated)))
...
def test_per_draft_and_per_attempt_aggregate_bounds() -> None:
    big = "x" * (MAX_AGGREGATE_BYTES + 1)
    with pytest.raises(SessionValidationError, match="1 MiB"):
        encode_session(_session(draft=big))
    turn = _turn(answer=big)
    with pytest.raises(SessionValidationError, match="1 MiB"):
        encode_session(_session(attempts=(turn,)))
    tools = {"blob": "y" * (MAX_AGGREGATE_BYTES + 1)}
    sneaky = _turn(answer="", tool_calls=(tools,), outcome="tool_only")
    with pytest.raises(SessionValidationError, match="1 MiB"):
        encode_session(_session(attempts=(sneaky,)))

    exact = "z" * MAX_AGGREGATE_BYTES
    kept = load_session(save_session(_session(draft=exact)))
...
def test_two_revision_barrier_keeps_newer_bytes() -> None:
    ...
    save_session(older)
    save_session(newer)
    with pytest.raises(SessionStaleWriteError):
        save_session(older)
    target = session_dir() / f"{session_id}.json"
    assert target.read_text(encoding="utf-8") == encode_session(newer)
    assert load_session(target).updated_at == T2
...
def test_write_failure_preserves_prior_bytes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(SessionPersistenceError):
        save_session(dataclasses.replace(session, draft="lost", updated_at=T2))
...
def test_post_replace_sync_failure_leaves_bytes_unconfirmed(
    ...
    with pytest.raises(SessionDurabilityUnconfirmed) as excinfo:
...
def test_private_permissions() -> None:
    target = save_session(_session(draft="secret"))
...
def test_owner_delete_under_held_lock() -> None:
    ...
    target = save_session(session)
    with lock_session(session.session_id) as handle:
        assert handle.held
        delete_session(session.session_id, lock_handle=handle)
        assert not target.exists()
        lock_file = target.with_suffix(".lock")
        assert lock_file.exists()
    with pytest.raises(SessionPersistenceError, match="unknown session"):
...
def test_two_app_running_read_only_lifecycle(
    ...
    with lock_session(session.session_id) as owner:
        viewer = load_session(target)
        assert viewer.attempts[0].outcome == "running"
        assert viewer.draft == "unsent ✎"
        owner_view = load_session(target, lock_handle=owner)
        assert owner_view.attempts[0].outcome == "interrupted"
        assert request_messages(viewer) == []
    assert load_session(target).attempts[0].outcome == "interrupted"
...
def test_symlinked_and_non_regular_files_rejected(tmp_path: Path) -> None:
    ...
    outside.write_text(encode_session(_session()), encoding="utf-8")
    link_id = str(uuid.uuid4())
    link = session_dir() / f"{link_id}.json"
    link.symlink_to(outside)
    with pytest.raises(SessionValidationError, match="non-regular"):
        load_session(link)
    directory = session_dir() / f"{uuid.uuid4()}.json"
    directory.mkdir()
    with pytest.raises(SessionValidationError, match="non-regular"):
        load_session(directory)
    with pytest.raises(SessionValidationError, match="non-regular"):
        save_session(dataclasses.replace(session, session_id=link_id))
...
def test_symlinked_sessions_root_refused(
    ...
    with pytest.raises(SessionPersistenceError, match="symlinked"):
        save_session(_session())


def test_invalid_ids_rejected() -> None:
    with pytest.raises(SessionValidationError):
        save_session(_session(session_id="nope"))
    with pytest.raises(SessionValidationError):
        delete_session("../escape")
    with pytest.raises(SessionValidationError):
        load_session(session_dir() / "ghost.json")
