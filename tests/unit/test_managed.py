...
def _inspection_payload(*, mlx_version: str = managed.MLX_VERSION) -> dict[str, object]:
    packages: dict[str, str] = {}
    for name, (version, _) in managed._freeze_requirements(
        managed._resource_bytes(managed.RUNTIME_RESOURCE).decode()
    ).items():
        packages[name] = version or managed.MLX_LM_VERSION
    packages["mlx"] = mlx_version
    return {
        "python_version": managed.PYTHON_VERSION,
        "packages": packages,
        "direct_urls": {"mlx-lm": {"vcs_info": {"commit_id": managed.RUNTIME_COMMIT}}},
...
def test_runtime_root_uses_versioned_xdg_data_path(
    ...
    assert (
        managed.runtime_root() == tmp_path / "mlx-tui" / "runtimes" / "74e7cf9-py3131"
    )
...
def test_sanitized_environment_drops_redirectors_but_keeps_proxy(
    ...
    env = managed._sanitized_environment()
...
def test_index_override_is_explicitly_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(managed.ManagedRuntimeError, match="index override"):
...
def test_preflight_rejects_wrong_platform(
    ...
    monkeypatch.setattr(managed.platform, "system", _linux)
    with pytest.raises(managed.ManagedRuntimeError, match="Darwin arm64"):
        managed._preflight(tmp_path / "runtime")
...
def test_inspect_runtime_rejects_version_mismatch(
    ...
    python.touch()
    monkeypatch.setattr(managed, "_uv_executable", _fake_uv)
    monkeypatch.setattr(managed, "_validate_profiles", _noop_validate)
    ...
    with pytest.raises(managed.ManagedRuntimeError, match="expected 0.32.2"):
        managed.inspect_runtime(root)
...
def test_inspect_runtime_rejects_direct_url_commit_mismatch(
    ...
    with pytest.raises(managed.ManagedRuntimeError, match="direct URL commit"):
        managed.inspect_runtime(root)
...
def test_install_targets_final_interpreter_and_writes_completion(
    ...
    root = tmp_path / "data" / "mlx-tui" / "runtimes" / managed.RUNTIME_DIRNAME

    def fake_runtime_root() -> Path:
        return root

    monkeypatch.setattr(managed, "runtime_root", fake_runtime_root)
    monkeypatch.setattr(managed, "_preflight", _fake_preflight)
    monkeypatch.setattr(managed, "inspect_runtime", _fake_inspection)
    ...
    monkeypatch.setattr(managed, "_run_install_command", fake_run)
    result = managed.install_runtime(
    ...
    assert (root / managed.COMPLETION_MARKER).exists()
...
def test_cancelled_install_leaves_incomplete_owned_runtime(
    ...
    root = tmp_path / "data" / "mlx-tui" / "runtimes" / managed.RUNTIME_DIRNAME
    monkeypatch.setattr(managed, "runtime_root", lambda: root)
    monkeypatch.setattr(managed, "_preflight", _fake_preflight)
    ...
    def cancel(
        ...
        *,
        ...
        raise managed.InstallCancelled("cancelled")

    monkeypatch.setattr(managed, "_run_install_command", cancel)
    with pytest.raises(managed.InstallCancelled):
        managed.install_runtime(
            on_line=lambda line: None, cancel_event=threading.Event()
        )
    assert (root / managed.OWNER_MARKER).exists()
    assert not (root / managed.COMPLETION_MARKER).exists()
...
def test_repair_refuses_unowned_existing_directory(
    ...
    monkeypatch.setattr(managed, "runtime_root", lambda: root)
    monkeypatch.setattr(managed, "_preflight", _fake_preflight)
    with pytest.raises(managed.ManagedRuntimeError, match="not app-owned"):
        managed.install_runtime(
            on_line=lambda line: None, cancel_event=threading.Event()
        )
...
def test_install_lock_reports_busy(
    ...
    root = tmp_path / "runtime"
    assert managed.fcntl is not None
    flock_module = managed.fcntl
    ...
    with pytest.raises(managed.ManagedRuntimeError, match="already busy"):
        with managed._runtime_lock(root):
            pass
...
def test_managed_runtime_retains_and_closes_its_child(
    ...
    root = tmp_path / "data" / "mlx-tui" / "runtimes" / managed.RUNTIME_DIRNAME
    root.mkdir(parents=True)
    (root / managed.OWNER_MARKER).write_text(
        json.dumps(
            {
                "schema_version": 1,
    ...
    def fake_port(_self: managed.ManagedRuntime) -> bool:
        return False

    def fake_listener(_self: managed.ManagedRuntime) -> bool:
        return True
    ...
    monkeypatch.setattr(managed, "inspect_runtime", fake_inspect)
    monkeypatch.setattr(managed, "verify_cached_assets", fake_verify)
    monkeypatch.setattr(managed.ManagedRuntime, "_port_is_occupied", fake_port)
    monkeypatch.setattr(
        managed.ManagedRuntime, "_listener_matches_child", fake_listener
    )
    ...
    runtime = managed.ManagedRuntime(root, port=18080)
    try:
...
def test_failed_managed_start_releases_runtime_lock(
    ...
    monkeypatch.setattr(managed.serverctl, "wait_healthy", failed_wait)
    runtime = managed.ManagedRuntime(root)
    try:
...
def test_managed_repeat_activation_requires_fresh_probe(
    ...
    monkeypatch.setattr(managed.serverctl, "spawn_command", fake_spawn)
    monkeypatch.setattr(managed.serverctl, "wait_healthy", fake_wait)
    runtime = managed.ManagedRuntime(root, port=18080)
    try:
...
def test_managed_repeat_activation_failure_preserves_child(
    ...
    try:
        runtime.start(snapshot, on_line=lambda _line: None)
        with pytest.raises(managed.ManagedRuntimeError, match="did not re-verify"):
            runtime.start(snapshot, on_line=lambda _line: None)
        assert runtime.process is child
        assert child.poll() is None
        assert spawns == 1
...
def test_managed_repeat_activation_cancelled_preserves_child(
    ...
    try:
        runtime.start(snapshot, on_line=lambda _line: None)
        with pytest.raises(managed.ManagedRuntimeError, match="cancelled"):
            runtime.start(snapshot, on_line=lambda _line: None)
        assert runtime.process is child
        assert child.poll() is None
    finally:
